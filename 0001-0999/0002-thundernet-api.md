...
For the purposes of secure and convenient encryption and decryption[F2], we will be using AES in Counter Mode[F3] with a 64-bit key.
...
Since the counter must be sent alongside the encrypted resource data, the counter should be sent as a hexadecimal-encoded string as header `Tn-Encryption-Counter`.
