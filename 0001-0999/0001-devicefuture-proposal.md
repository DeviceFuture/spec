...
One of the fundamental missions of DeviceFuture is to ensure that everybody has access to an internet which is free of large-scale censoring that promotes the interests of one party only. Our combined use of a decentralised, multi-node network and symmetrical encryption can help to guarantee the privacy of a user who would otherwise be using potentially a restricted and monitored network. We plan to use AES encryption since it is built into all modern browsers[F2] and therefore does not need external library support (which in turn, reduces data transmission when installing our software on endpoint devices).
...
We plan to use a JavaScript port[F3] of ZXing to scan QR codes and QRCode.js[F4] to generate QR codes. Both actions will be done from within ThunderNet Browser on endpoints.
...
To ensure that ThunderSearch is efficient, rendering of the search engine will happen within ThunderNet Browser within the endpoint. This means that a ThunderNet node only needs to send search results through a simple API. We plan to use NeDB[F5], a lightweight and fast database system, to store ThunderNet resources in addition to ThunderSearch results.

## Footnotes
[F1] https://github.com/LZMA-JS/LZMA-JS

[F2] https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto

[F3] https://github.com/nimiq/qr-scanner

[F4] https://github.com/davidshimjs/qrcodejs

[F5] https://github.com/louischatriot/nedb
